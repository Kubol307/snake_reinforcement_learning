...
class GameEnv(gym.Env):
    def __init__(self, train=True):
        ...
        self.observation_space = spaces.Box(low=0, high=255, shape=(3, 1000, 1000), dtype=np.uint8)
    ...
    def step(self, action):
        ...
        img = self._game_image(self.apple, self.snake)
        # set observation
    ...
    def _game_image(self, apple, snake):
        img = np.zeros((3, 1000, 1000), dtype=np.uint8)
        img[:, apple.position[0], apple.position[1]] = [255, 255, 0]
        for position in snake.positions[1:]:
            img[:, position[0], position[1]] = [0, 255, 0]
        try:
          img[:, snake.positions[0][0], snake.positions[0][1]] = [255, 0, 0]
        except:
          pass
        return img
...
def main():
    ...
    model = PPO('CnnPolicy', env, verbose=1, learning_rate=0.001, tensorboard_log=tensorboard_dir)
    # model = PPO.load('/home/jakub/python/reinforcement_learning/snake_rl/models/1700948034/0', env, tensorboard_log=tensorboard_dir)
