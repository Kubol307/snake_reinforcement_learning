...
class ANN():
    ...
    def train_batch_gd_with_momentum(self, learning_rate=1e-5, reg=0.1, epochs=10_000, batch_size=64):
        W1 = self.W1.copy()
        b1 = self.b1.copy()
        W2 = self.W2.copy()
        b2 = self.b2.copy()
        ...
        print(f'best validation error: {best_validation_error}, cost: {costs[-1]}') 
        print('Cost per second', cost_difference/full_time,)  
        plt.plot(costs, label="momentum")
        # plt.show()

    def train_batch_gd_nesterov_momentum(self, learning_rate=1e-5, reg=0.1, epochs=10_000, batch_size=64, label='nesterov'):
        W1 = self.W1.copy()
        b1 = self.b1.copy()
        W2 = self.W2.copy()
        b2 = self.b2.copy()
    ...
    def train_sgd_nesterov(self, learning_rate=1e-5, reg=0.1, epochs=10_000, label='sgd nesterov'):
        costs = []
        best_validation_error = 0
        ...
        for epoch in range(epochs):
            for n in range(500):
                ...
                gW2 = learning_rate*(Z.T.dot(pY_Y) + reg*W2)
                gb2 = learning_rate*((pY_Y).sum(axis=0) + reg*b2)
                gW1 = learning_rate*((x.T.dot((pY_Y).dot(self.W2.T)*(Z > 0))) + reg*W1)
                gb1 = learning_rate*(((pY_Y).dot(W2.T)*(Z > 0)).sum(axis=0) + reg*b1)

                vW2 = vW2*mu - learning_rate*gW2
                vb2 = vb2*mu - learning_rate*gb2
                vW1 = vW1*mu - learning_rate*gW1
                vb1 = vb1*mu - learning_rate*vb1
...
# ann.train_batch_gd_nesterov_momentum_RMS(epochs=100, batch_size=64, learning_rate=1e-3, label='RMS')
